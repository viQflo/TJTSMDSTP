	...
	<body>
	    ...
	    <div id="board-container" class="min-h-screen flex flex-col items-center justify-center bg-white p-6">
	        ...
	        <div class="flex justify-center items-center mb-8 gap-2">
	            <select id="search-filter" class="border border-gray-300 rounded-l-lg px-4 py-2">
	            ...
	            <button id="search-button" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">검색</button>
	        </div>
	
	        <!-- 게시글 목록 -->
	        <div id="posts-list" class="w-full max-w-4xl">
	            <% for(BoardDTO post : postList) { 
	                String postFile = (post.getPostFile() != null && !post.getPostFile().isEmpty()) ? post.getPostFile() : "https://cdn-icons-png.flaticon.com/512/3282/3282224.png";
	            %>
	                <div class="post-item" onclick="location.href='BoardDetail.jsp?postIdx=<%= post.getPostIdx() %>'; ">
	                    <img src="<%= postFile %>" alt="Thumbnail" class="post-thumbnail">
	                    <div class="post-info">
	                        <div class="post-title"><%= post.getPostTitle() %></div>
	                        <div class="post-meta">
	                            작성자: <%= post.getEmail() %> | 조회수: <%= post.getPostViews() %> | 좋아요: <%= post.getPostLikes() %>
	                        </div>
	                    </div>
	                </div>
	            <% } %>
	        ...
	        <!-- 페이지네이션 -->
	        <div id="pagination-buttons" class="flex justify-center space-x-2 mt-6">
	            <% if (pageNum > 1) { %>
	                <a href="Board.jsp?pageNum=<%= pageNum - 1 %>" class="page-button">이전</a>
	            <% } %>
	
	            <% for (int i = 1; i <= totalPages; i++) { %>
	                <a href="Board.jsp?pageNum=<%= i %>" class="page-button <%= (i == pageNum) ? "active" : "" %>">
	                    <%= i %>
	                </a>
	            <% } %>
	
	            <% if (pageNum < totalPages) { %>
	                <a href="Board.jsp?pageNum=<%= pageNum + 1 %>" class="page-button">다음</a>
	            <% } %>
	        </div>
	
	        <!-- 글쓰기 버튼 -->
	        <button id="write-post-button" class="bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600 mt-4">
	            글쓰기
	        </button>
	    </div>
